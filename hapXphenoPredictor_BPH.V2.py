...
def fnc(msu, dat):
    ...
    pos_cnts = vcf.groupby(['POS','REF','ALT']).size().reset_index().rename(columns={0:'cnt'})
    pos_cnts2 = pos_cnts.groupby(['POS']).size().reset_index().rename(columns={0:'cnt'})
    multi = pos_cnts2[pos_cnts2.cnt > 1].POS.unique()

    for mult in multi:
        #alts = vcf[vcf.POS == mult].sort_values('ALT').ALT.values #RM!!!!!!!!!!!!!!!!!!!!!!
        alts = pos_cnts[pos_cnts.POS == mult].sort_values('ALT')
        cnt = 1
        for i in alts.index:
            try:
                snps[1][(snps[1] == mult) & (snps[3] == alts.REF.loc[i]) & (snps[4] == alts.ALT.loc[i])] = f"{mult}_{cnt}"
            except:
                pass
            cnt += 1
    ...
    posn_samps.marker = posn_samps.marker.astype(str) #ADDED !!!!!!!!!!!!!!!!!!!!!!
    ...
    sign.iloc[:, -1:] = sign.iloc[:, -1:].applymap(lambda x: round(x, 2))
