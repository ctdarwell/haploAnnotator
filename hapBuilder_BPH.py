...
#START
fldr = sys.argv[1] #'ayt_day9'
out_dir = sys.argv[2] #'figures'
...
for file in files:
    ...
    df['hap'] = df[df.columns[1:-1].tolist()].agg(' '.join, axis=1) #compress haps
    ...
    snpsDF = df4[cols + ['hap_no','counts','val']]
    ...
    for indx in snpsDF.index:
        val = snpsDF.val[indx]
        ci = (df.val.mean(), df.val.mean())
        for r in range(5, 105, 5):
            if val > ci[0] and val < ci[1]:
                break
            ci = norm(*norm.fit(df.val)).interval(r/100)
        snpsDF.perc[indx] = f"{r - 5}th"
    ...
    for col in snpsDF.columns[:-4]: #cycle thru rows (i.e. haps)        
        tmp = snpsDF[col]
    ...
    colours.extend([snpsDF.shape[0] * ['pink']] * 4)
    colours = np.array(colours).T
    
    #tidy up
    vcf = pd.read_csv(f"{fldr}/{file.replace('haps.csv','vcf')}", sep='\t', header=0)
    chrom = vcf['#CHROM'][0] #THIS LINE BETTER IN SW100 VERSION
    new_cols = []
